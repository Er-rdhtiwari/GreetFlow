...
TAG="${1:?Usage: deploy_dev.sh <image_tag>}"
AWS_REGION="${AWS_REGION:-ap-south-1}"
CLUSTER="${DEV_CLUSTER:-greetflow-dev-eks}"
NS="greetflow-dev"
...
# You can set ACM_ARN from terraform output:
# ACM_ARN=$(cd infra/terraform/envs/dev && terraform output -raw acm_cert_arn)
...
helm upgrade --install greetflow-dev infra/helm/webapp \
  -n "${NS}" \
